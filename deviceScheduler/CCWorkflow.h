...
class CCWorkflow {
    ...
    unsigned char        countOfTasks[10];
    
    /// Parameters of the device as a peer of a run-loop.
    /// Value, that holds the index of the current task.
    unsigned char        taskPointer[10];
...
public:
    ...
    /// @sa countOfTasks;
    unsigned char getCountOfTasks(schedulerDevice device);
    
    /// Setter method for manipulating the number of tasks of the device
    /// @sa countOfTasks;
    void setCountOfTasks(schedulerDevice device, unsigned char count);
    
    /// Getter method for getting the taskPointer of the device
    /// @sa taskPointer;
    unsigned char getTaskPointer(schedulerDevice device);
    
    /// Setter method for setting the taskPointer of the device
    /// @sa taskPointer;
    void setTaskPointer(schedulerDevice device, unsigned char pointer);
    
    /// Setter method for increasing the taskPointer of the device
    /// @sa taskPointer;
    void increaseTaskPointer(schedulerDevice device);
